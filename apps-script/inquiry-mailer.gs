/**
 * 台灣營德官網「留下聯絡資料」接收端
 * 功能：把表單寫進 Google Sheet + 寄出通知信，並回傳 JSON 讓前端能分辨成功／失敗
 *
 * 部署步驟
 ...
 *    - 執行身分：我
 *    - 具有存取權的使用者：所有人
 * 4. 複製產生的 /exec 網址，設成 Cloudflare Pages 的環境變數 GAS_URL
...
/* 寄件人＝部署這支腳本的帳號（inteplasttaiwanc@gmail.com）。
   ...
   收通知信的信箱不寫死在這裡：送出時會一併帶上頁面上「聯絡我們」
   顯示的那個信箱（notifyTo 欄位，來源是後台的公司資訊），
   所以之後只要改後台，通知信就跟著換，不必重新部署這支腳本。
   下面的 NOTIFY_TO 只是萬一沒帶到時的備援。
   ALLOWED_NOTIFY_DOMAINS 是白名單：避免有人偽造 notifyTo 把這支腳本當成轉信機。 */
const NOTIFY_TO = 'inteplasttw@fpc.com.tw';
const ALLOWED_NOTIFY_DOMAINS = ['fpc.com.tw', 'wpjk.inteplast.com', 'inteplast.com', 'gmail.com'];
