...
Sheet），現在跟客戶自動回覆一樣走 Resend。Apps Script 整個不再使用。
  ...
  業務通知信   寄件人 inquiry@inteplasttw.com.tw
               收件人＝後台「公司資訊」的聯絡信箱 + 副本欄（email_cc）
               ※ 每位收件人各寄一封、都放在 To，不用 CC ——
                 企業郵件閘道會把「不在 To 欄」的外部信靜默隔離，
                 Resend 顯示 delivered 但收件匣、垃圾郵件都找不到。
               回覆地址＝客戶信箱，按回覆就是回客戶
               信裡有客戶填的內容、送出時間、紀錄編號，
...
3) 額度 = 業務收件人數 + 1（客戶回覆）
   例：2 位業務 → 一筆詢價 3 封。
   Resend 免費版每天 100 封／每月 3,000 封 → 每天約 30 筆詢價。
